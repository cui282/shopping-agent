...
Clients must branch on `event`; `message` is display text and may change. Events generated before the WebSocket connects are buffered and replayed in order. A WebSocket for an unknown or already deleted task is rejected with close code `1008`.
