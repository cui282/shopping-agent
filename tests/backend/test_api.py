...
from app.schemas import ShoppingSummaryOutput, TaskSnapshot
...
def test_task_validates_and_exposes_upload_references(client: TestClient) -> None:
    ...
    assert first["event"] == "session_created"
    assert first["data"]["reference_images"][0]["upload_id"] == upload["upload_id"]
...
async def test_cancel_converges_task_cancelled_before_coroutine_starts() -> None:
    ...
    server.records[thread_id] = server.TaskRecord(
        run_id="cancel-before-start-run",
        snapshot=snapshot,
        task=task,
    )
...
async def test_completed_snapshot_cannot_be_rolled_back_by_cancel(monkeypatch) -> None:
    ...
    original_emit = server.monitor.emit
    ...
    async def delayed_emit(thread_id, event, **kwargs):
        if event == "task_result":
            terminal_emit_started.set()
            await release_terminal_emit.wait()
        return await original_emit(thread_id, event, **kwargs)

    monkeypatch.setattr(server, "run_agent", completed_agent)
    monkeypatch.setattr(server.monitor, "emit", delayed_emit)
...
def test_websocket_send_failure_does_not_change_successful_task(client: TestClient) -> None:
